    ...
    public TopicConfigManager(RocketMQBrokerController brokerController) {
        ...
        int defaultPartitionNum = config.getDefaultNumPartitions();
        {
            ...
            if (this.brokerController.getServerConfig().isAutoCreateTopicEnable()) {
                String topic = RocketMQTopic.getPulsarMetaNoDomainTopic(RopMixAll.AUTO_CREATE_TOPIC_KEY_TOPIC);
        ...
        }
        {
            ...
            String topic = RocketMQTopic.getPulsarMetaNoDomainTopic(RopMixAll.BENCHMARK_TOPIC);
            this.systemTopicList.add(topic);
//            TopicConfig topicConfig = new TopicConfig(topic);
...
//            this.topicConfigTable.put(topicConfig.getTopicName(), topicConfig);
        ...
        }
        {
            ...
            String topic = RocketMQTopic.getPulsarMetaNoDomainTopic(RopMixAll.OFFSET_MOVED_EVENT);
            TopicConfig topicConfig = new TopicConfig(topic);
            this.systemTopicList.add(topic);
                ...
                try {
                    ...
                    if (defaultTopicConfig != null) {
                        if (pulsarDefaultTopic
                                .equals(RocketMQTopic.getPulsarMetaNoDomainTopic(RopMixAll.AUTO_CREATE_TOPIC_KEY_TOPIC))) {
                            if (!this.brokerController.getServerConfig().isAutoCreateTopicEnable()) {
    ...
    public TopicConfig createTopicOfTranCheckMaxTime(final int clientDefaultTopicQueueNums, final int perm) {
        TopicConfig topicConfig = this.sysTopicConfigTable.get(RopMixAll.TRANS_CHECK_MAX_TIME_TOPIC);
        if (topicConfig != null) {
            return topicConfig;
        }
        ...
        try {
            if (this.lockTopicConfigTable.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    topicConfig = this.sysTopicConfigTable.get(RopMixAll.TRANS_CHECK_MAX_TIME_TOPIC);
                    if (topicConfig != null) {
                        return topicConfig;
                    }

                    topicConfig = new TopicConfig(RopMixAll.TRANS_CHECK_MAX_TIME_TOPIC);
                    topicConfig.setReadQueueNums(clientDefaultTopicQueueNums);
                    topicConfig.setWriteQueueNums(clientDefaultTopicQueueNums);
                    topicConfig.setPerm(perm);
                    ...
                    this.dataVersion.nextVersion();
                    this.createPulsarPartitionedTopic(topicConfig);
                    this.sysTopicConfigTable.put(RopMixAll.TRANS_CHECK_MAX_TIME_TOPIC, topicConfig);
                } finally {
                    this.lockTopicConfigTable.unlock();
                }
            }
