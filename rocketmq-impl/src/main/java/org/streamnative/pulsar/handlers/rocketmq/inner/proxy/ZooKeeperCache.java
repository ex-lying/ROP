...
public abstract class  ZooKeeperCache implements Watcher {
    ...
    public <T> void process(WatchedEvent event, final CacheUpdater<T> updater) {
        ...
        if (path != null) {
            ...
            if (event.getType().equals(Event.EventType.NodeCreated) || event.getType().equals(Event.EventType.NodeDeleted)) {
                childrenCache.synchronous().invalidate(getParentForPath(path));
            }
    ...
    public <T> Optional<Map.Entry<T, Stat>> getEntry(final String path, final Deserializer<T> deserializer) throws Exception {
        return getData(path, this, deserializer);
    }

    public <T> CompletableFuture<Optional<Map.Entry<T, Stat>>> getEntryAsync(final String path, final Deserializer<T> deserializer) {
        CompletableFuture<Optional<Map.Entry<T, Stat>>> future = new CompletableFuture<>();
     ...
     * @throws Exception
     */
    public <T> Optional<Map.Entry<T, Stat>> getData(final String path, final Watcher watcher,
    ...
    public <T> CompletableFuture<Optional<Map.Entry<T, Stat>>> getDataAsync(final String path, final Watcher watcher,
        ...
        dataCache.get(path, (p, executor) -> {
            ...
            CompletableFuture<Pair<Map.Entry<Object, Stat>, Long>> zkFuture = new CompletableFuture<>();
                    ...
                    if (rc == KeeperException.Code.OK.intValue()) {
                        try {
                            ...
                            executor.execute(() -> zkFuture.complete(ImmutablePair
                                    .of(new AbstractMap.SimpleImmutableEntry<Object, Stat>(obj, stat), System.nanoTime())));
                        } catch (Exception e) {
            ...
            if (result != null) {
                future.complete(Optional.of((Map.Entry<T, Stat>) result.getLeft()));
            } else {
                future.complete(Optional.empty());
            }
        }).exceptionally(ex -> {
    ...
    private <T> void checkAndRefreshExpiredEntry(String path, final Deserializer<T> deserializer) {
        CompletableFuture<Pair<Map.Entry<Object, Stat>, Long>> result = dataCache.getIfPresent(path);
        if (result != null && result.isDone()) {
            Pair<Map.Entry<Object, Stat>, Long> entryPair = result.getNow(null);
    ...
    public <T> T getDataIfPresent(String path) {
        CompletableFuture<Pair<Map.Entry<Object, Stat>, Long>> f = dataCache.getIfPresent(path);
