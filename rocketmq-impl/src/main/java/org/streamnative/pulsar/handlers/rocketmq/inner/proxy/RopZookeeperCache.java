...
public class RopZookeeperCache extends ZooKeeperCache implements Closeable {
    ...
    public void start() throws RopServerException {
        if (!isRunning) {
            try {
                CompletableFuture<ZooKeeper> zkFuture = zlClientFactory.create(ropZkConnect, ZooKeeperClientFactory.SessionType.ReadWrite,
    ...
    protected void asyncRestartZooKeeperSession() {
        ...
        CompletableFuture<ZooKeeper> zkFuture = zlClientFactory.create(ropZkConnect, ZooKeeperClientFactory.SessionType.AllowReadOnly,
                zkSessionTimeoutMillis);
