...
/*    private final Meter sendSingleMeter = this.newMeter("rop.send.single", "", TimeUnit.SECONDS, null);
    ...
    private RemotingCommand handlePutMessageResult(org.apache.rocketmq.store.PutMessageResult putMessageResult,
        ...
        if (sendOK) {
            ...
            this.brokerController.getBrokerStatsManager()
                    .incBrokerPutNums(msg.getTopic(), putMessageResult.getAppendMessageResult().getMsgNum());
            ...
            if (hasSendMessageHook()) {
                ...
                int incValue = (int) Math.ceil(wroteSize / SIZE_PER_COUNT) * commercialBaseCount;
            ...
            if (hasSendMessageHook()) {
                ...
                int incValue = (int) Math.ceil(wroteSize / SIZE_PER_COUNT);
