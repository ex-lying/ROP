...
public class RocketMQStandalone implements AutoCloseable {
    ...
    public void start() throws Exception {
        ...
        if (!this.isNoFunctionsWorker()) {
            ...
            //pulsar 3.0.6版本该方法已经弃用
            //workerConfig.setConfigurationStoreServers(config.getConfigurationStoreServers());
            workerConfig.setConfigurationStoreServers(config.getConfigurationMetadataStoreUrl());
        ...
        if (config.isTransactionCoordinatorEnabled()) {
            /**
             * pulsar 3.0.6版本将该api改为openTransactionMetadataStore
             */
            //broker.getTransactionMetadataStoreService().addTransactionMetadataStore(TransactionCoordinatorID.get(0));
            TransactionTimeoutTrackerFactory timeoutTrackerFactory = new TransactionTimeoutTrackerFactoryImpl(
                    broker.getTransactionMetadataStoreService(),
                    new HashedWheelTimer(new DefaultThreadFactory("pulsar-transaction-timer"))
            );

            TransactionTimeoutTracker timeoutTracker = timeoutTrackerFactory.newTracker(TransactionCoordinatorID.get(0));
            TransactionRecoverTracker recoverTracker =
                    new TransactionRecoverTrackerImpl(broker.getTransactionMetadataStoreService(),
                            timeoutTracker, TransactionCoordinatorID.get(0).getId());

            broker.getTransactionMetadataStoreService().openTransactionMetadataStore(TransactionCoordinatorID.get(0),
