...
public class RocketMQBrokerController {
    ...
    public RocketMQBrokerController(final RocketMQServiceConfiguration serverConfig) throws PulsarServerException {
        ...
        this.brokerStatsManager = new BrokerStatsManager(serverConfig.getBrokerName(), false);
    ...
    public void initialize() throws Exception {
        ...
        this.msgCallbackExecutor = new ThreadPoolExecutor(
        ...
        this.queryMessageExecutor = new ThreadPoolExecutor(
        ...
        this.endTransactionExecutor = new ThreadPoolExecutor(
        ...
        this.ropBrokerRequestExecutor = new ThreadPoolExecutor(
                this.serverConfig.getRopBrokerRequestThreadPoolCapacity(),
                this.serverConfig.getRopBrokerRequestThreadPoolCapacity(),
                1000 * 60,
                TimeUnit.MILLISECONDS,
            ...
            private boolean checkTokens(String token, String topic, TopicOperation authOp) {
                ...
                if (authCaches.getIfPresent(authTriple) == null) {
                    try {
                        ...
                        /**
                         * 由于org.apache.pulsar.broker.authentication.AuthenticationService.authenticate方法3.0.6不再提供
                         * 改为使用org.apache.pulsar.broker.authentication.AuthenticationProvider.authenticateAsync方法
                         */
                        //String roleSubject = authService.authenticate(authCommand, "token");

                        AuthenticationProvider provider = authService.getAuthenticationProvider("token");
                        String roleSubject = provider.authenticateAsync(authCommand).get();
...
//                        || RequestCode.CONSUMER_SEND_MSG_BACK == request.getCode()
    ...
    private void initialTransaction() {
        this.transactionalMessageService = ServiceProvider
                .loadClass(TransactionalMessageService.class);
        ...
        this.transactionalMessageCheckListener = ServiceProvider
                .loadClass(AbstractTransactionalMessageCheckListener.class);
